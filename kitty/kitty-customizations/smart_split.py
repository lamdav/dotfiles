...
class KittySmartSplitter:
    ...
    def smart_split(self) -> bool:
        ...
        print(
            f"📐 Current window: {columns}x{lines} (aspect ratio: {columns/lines:.2f})"
        )
        ...
        try:
            if self.should_split_vertically(columns, lines):
                ...
                subprocess.run(
                    ["kitty", "@", "launch", "--location=vsplit", "--cwd", cwd],
                    check=True,
                )
                ...
                subprocess.run(
                    ["kitty", "@", "launch", "--location=hsplit", "--cwd", cwd],
                    check=True,
                )
                ...
                subprocess.run(["kitty", "@", "new-tab", "--cwd", cwd], check=True)

...
class KittyNotifier:
    ...
    def monitor_command(self, command: list, notify_on_completion: bool = True):
        ...
        try:
            ...
            if notify_on_completion and (
                is_long_running or has_errors or result.returncode != 0
            ):
            ...
            self.send_notification(
                "⏹️ Command Interrupted", f"'{command_str}' was interrupted"
            )
            return 130

        except Exception as e:
            self.send_notification(
                "💥 Command Error", f"Error running '{command_str}': {e}"
            )
            return 1
    ...
    def notify_on_background_job_completion(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["jobs"], shell=True, capture_output=True, text=True
            )
            ...
            if result.stdout.strip():
                ...
                for line in lines:
                    if "Done" in line or "Exit" in line:
                        self.send_notification(
                            "🎯 Background Job Complete", f"Job finished: {line}"
                        )
...
def main():
    ...
    if sys.argv[1] == "--jobs":
        ...
        notifier.send_notification(
            "🧪 Test Notification", "Kitty notification system is working!"
        )
